    ...
    *
    ...
    public function adminsend($post)
    {
        ...
        if($id){
            ...
            if($post['accept']==1)
            {
                $modelmsg = D('TblSystemMsg2');
                $sql  = "SELECT max(userid) AS num FROM cj_system_msg2.cj_sys_msg_all_users LIMIT 0,1 ";
                ...
                if($have[0]['num'] == ''){
                    ...
                    $addsql = "INSERT INTO cj_system_msg2.cj_sys_msg_all_users (`userid`) VALUES ".$insertval;
            ...
            elseif($post['accept']==2)
            {
                ...
                $Message->systemMessage($userar,trim($post['content']));
            }
            // 每天群发消息
            elseif($post['accept']==4)
            {
                ...
                $userar[] = 1000035;//测试接受 徐鸣
                $rds->hSet($redis_key,'ltime',$now);
                return $Message->systemMessage($userar,trim($post['content']));
                //var_dump([$ltime,$arr]);die;
                return true;
            }
            else
            {
                $userar = explode(',',$post['accept_uid']);
                return $Message->systemMessage($userar,trim($post['content']));
            }
        }else{
            $this->error('添加失败');

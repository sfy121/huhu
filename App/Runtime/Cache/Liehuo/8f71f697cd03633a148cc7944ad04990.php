...
<div id="content">
  <div class="container-fluid">
    <div class="row-fluid">
      <form action="<?php echo U();?>" method="GET" class="form-inline" style="margin-bottom:20px;">
...
<?php if(is_array($sexs)): $i = 0; $__LIST__ = $sexs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>"<?php echo $_REQUEST['sex'] == (string)$key ? ' selected' : ''; ?>><?php echo ($v); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
          </select>
        </div>
        <button type="submit" class="btn btn-primary">搜索</button>
        <span class="btn btn-white pull-right">记录数：<?php echo count($data['list']);?></span>
      </form>
      <div class="table-responsive">
        <table class="table table-bordered table-striped table-hover text-center">
          ...
          <tbody>
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; $par = [ 'stime' => $v['date'], 'etime' => $v['date'], ]; $sex = $_REQUEST['sex'] == '' ? '' : (int)$_REQUEST['sex']; ?>
            <tr>
              <td><a href="<?php echo U('UserBase/oper_logs?type=scoring&sex='.$sex,$par);?>" target="_blank"><?php echo ($v['date']); ?></a></td>
              <td><?php echo ($v['score0']); ?></td>
              <td><?php echo ($v['score5']); ?></td>
              <td><?php echo ($v['score6']); ?></td>
              <td><?php echo ($v['score7']); ?></td>
              <td><?php echo ($v['score8']); ?></td>
              <td><?php echo ($v['score9']); ?></td>
              <td><?php echo ($v['cnt_all']); ?></td>

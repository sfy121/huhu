    ...
    <div class="row-fluid">
      <div class="widget-box">
        ...
        <div class="widget-content nopadding">
          <div class="clearfix">
            <a href="<?php echo U('add');?>" class="btn btn-success pull-right">新增管理员</a>
          </div>
...
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr class="gradeX">
                  ...
                  <td title="<?php echo ($v['nickname']); ?>"><?php echo ($v['nickname']); ?></td>
                  <td><?php echo ($v['email']); ?></td>
                  <td><?php echo ($v['create_time'] ?: '-'); ?></td>
                  <td><?php echo ($v['last_login_time'] ?: '-'); ?></td>
                  <td>
                    <a href="<?php echo U('edit?id='.$v['aid']);?>" class="btn btn-primary btn-mini">修改</a>
                    <a href="<?php echo U('del?id='.$v['aid']);?>" class="btn btn-danger btn-mini" onclick="return confirm('确定这么做？')">删除</a>
                  </td>
